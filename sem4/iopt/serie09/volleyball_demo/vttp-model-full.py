...
#start modeling
#variables
model.x =  pyo.Var([(i,j,d) for i in teams.keys() for j in teams.keys() for d in match_days if i!=j], within = pyo.Binary) # 1 if a match between team i (at home) and team j happens on day d, 0 otherwise

#auxiliary variables
model.xh = pyo.Var([(i,d) for i in teams.keys() for d in match_days], within = pyo.Binary) # 1 if team i is home on day d, 0 otherwise
model.xa = pyo.Var([(i,d) for i in teams.keys() for d in match_days], within = pyo.Binary) # 1 if team i is away on day d, 0 otherwise
...
for i in teams:
    for d in range(1,q-1):
        if d in match_days and d+2 in match_days:
...
for i in teams:
    for d in range(1,q-5):
        if d in match_days and sum(1 for d2 in range(d,d+7) if d2 in match_days) >= 6:
